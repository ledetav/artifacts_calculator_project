...
@Entity(
    ...
    foreignKeys = [
        // Ссылка на энциклопедию оружия
        ForeignKey(
            entity = WeaponEntity::class,
            parentColumns = ["id"],
            childColumns = ["weapon_encyclopedia_id"],
            onDelete = ForeignKey.CASCADE
        ),
        // Ссылка на владельца (если есть)
        ForeignKey(
    ...
    indices = [
        Index(value = ["weapon_encyclopedia_id"]),
...
data class UserWeaponEntity(
    ...
    @ColumnInfo(name = "weapon_encyclopedia_id")
    val weaponId: Int,

    val level: Int,
    val ascension: Int, // 0-6
    val refinement: Int, // 1-5

    @ColumnInfo(name = "is_locked")
    val isLocked: Boolean,

    // Кто носит (может быть null, если лежит в инвентаре)
